...
const FeedbackModal: React.FC<FeedbackModalProps> = ({ boothId, onSubmit, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-[60] p-4" onClick={onClose}>
      <div className="relative bg-gray-800 rounded-2xl shadow-xl w-full max-w-md p-8 border border-gray-700" onClick={e => e.stopPropagation()}>
        <button
            type="button"
            onClick={onClose}
        ...
        <h2 className="text-2xl font-bold text-center mb-2">Feedback for <span className="text-purple-400">{boothId}</span></h2>
        <p className="text-center text-gray-400 mb-6">Your feedback helps us improve!</p>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2 text-center">1. How would you rate this demo?</label>
          ...
          <div>
            <label htmlFor="feedback" className="block text-sm font-medium text-gray-300">2. How could you apply this to your work?</label>
            <textarea
              id="feedback"
              rows={4}
              value={feedback}
